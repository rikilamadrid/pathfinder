...
describe("repairs from the 53.7 review", () => {
  ...
  it("gives a review brief review steps, never implementation ones", () => {
    ...
    assert.match(buildBrief({ ...BRIEF_BASE, session: "deploy", selection: { role: "developer", model: "inherited", effort: "inherited" } }).message, /session must be implementation, resume, rebase-and-reverify, resolve-conflict, review/);
  });
