...
- **`lib/`, the kit's first shared code directory.** Every destination
  ...
  first occupant is `lib/evidence-references.mjs`, the one statement of the
  `type:locator` evidence-reference grammar. `blog-post-redactor` imports and
  re-exports it from its own `sources.mjs`, which behaves exactly as before,
  and `reflect`'s improvement ledger will import the same file rather than
  depending on another skill. Nothing existing breaks and nothing needs
  migrating: a project that re-runs the installer gains the directory.
  A conformance fixture in `packages/evidence-references/` is run by every
  consumer's suite, so a disagreement about what the grammar accepts fails
  CI by the reference it disagreed on.
...
- **Visual artifacts, compiled rather than hand-authored.** A Pathfinder skill that delivers something visual no longer writes the page. It writes a small typed specification saying what is true — and `render-artifact` compiles that into one self-contained HTML file that opens straight from the filesystem with no build step, no server, and no network.

  The split is the point: the producer owns the content and what it claims, the renderer owns presentation, navigation, theming, and every word of interface language. A specification has no field for a colour, a coordinate, a size, a font, or a class, and the schema refuses such a field rather than quietly ignoring it.

  Evidence is part of the contract, not a convention. Every claim cites a path — optionally a line range — that has to resolve against a commit the specification names, never your working tree, so a later refactor cannot silently change what an artifact asserts. Output is deterministic on a versioned contract: the same specification and the same renderer version produce byte-identical HTML across machines, directories, timezones, and locales. Delivery returns a receipt naming the renderer version and the digests of both sides — and that receipt says the artifact was *checked*, never that it *looks right*. Whether it reads well is a person's judgement, reported separately.

  Generating an artifact needs Node. Nothing else in Pathfinder does, and the kit still installs no runtime into your application.

- **Semantic diagrams, and `map-system` to ask for one.** `render-artifact` now compiles two kinds: `lesson` and `diagram`. Ask `map-system` in plain words — map this repository's architecture, visualize this checkout flow, map the ticket lifecycle — and it produces a diagram you can interrogate.

  You describe the system, never the canvas: what exists, what each thing is, what relates to what, which things belong together, and which ordered walks matter. Rank, axis, position, size, spacing, edge routing, label placement, shape, colour and emphasis are all the renderer's, computed from the graph's own shape. Emphasis in particular is derived rather than requested — say why a walk matters, and the renderer decides how much ink it gets.

  **A diagram says where it came from.** A `derived` diagram maps what a repository asserts about itself at a pinned commit, and every component and every relationship in it carries a citation that resolved there. A `proposed` diagram describes a system that does not exist yet; with no repository behind it, its evidence layer is reported as *not run*, with the reason, rather than as passed — and it carries no verification sentence at all. The wording of each is the renderer's, and nothing in a specification can select it, soften it, or ask for one it has not earned.

  **A diagram is read, not just looked at.** Focus a component and walk upstream or downstream; highlight exactly the path its author drew; open the evidence behind any claim; pan, zoom, fit, reset, and do all of it from the keyboard. Traversal follows the relationships the producer wrote, never which boxes happen to sit nearby. With JavaScript switched off the artifact is still a complete, readable, navigable document — every fact, including every citation, is in the page before any script runs.

### Changed

- **`learn-feature` and `learn-codebase` produce their lessons through the shared renderer.** Neither skill authors HTML, CSS, presentation JavaScript, or MDX any more, and both go through the same contract with no consumer-specific field or escape hatch — `learn-feature` supplies one module, `learn-codebase` supplies many, and the renderer does not ask which.

  `learn-codebase` no longer builds a multi-file portal directory. How a lesson is split, linked, and navigated was always presentation, and it now produces a single artifact like everything else. It gains no diagrams of its own: diagrams come from `map-system`, as their own artifact.
...
- **The activation fragment, in the installer and on the site.** A run at a terminal that generates the handler now prints the exact `SessionStart` fragment that switches it on, correct for the stable path it just wrote, along with the answer for anyone who skips it: no automatic orientation, and `/whereami` on demand. A piped or redirected run prints none of it: that output is a byte promise kept to scripts written against 1.4.1, and a multi-line JSON fragment in the middle of somebody's parse is not a place to put a new fact. `.claude/settings.local.json` is the documented default, because turning orientation on should be one person's per-machine choice; `.claude/settings.json` is named only as the deliberate alternative for a team that wants it on for everyone. The two-step removal is stated in the same breath — delete the handler, drop the fragment — and so is the fact that doing only one is survivable in either order.

  [Session orientation](https://pathfinder-kit.vercel.app/guides/session-orientation/) is the long form. The fragment there is checked against the one the installer prints, so a guide that drifts fails the build rather than handing a reader a paste that quietly activates nothing.
