...
- **Native skill discovery for Claude Code, generated on request.** `create-pathfinder --agents claude-code` writes a small adapter at `.claude/skills/<name>/SKILL.md` for every Pathfinder skill, so they appear as native skills instead of a path you paste. In a terminal you are asked instead, with detected tools offered as the default; nothing is configured unless you choose it, and a non-interactive run configures nothing without `--agents`.
