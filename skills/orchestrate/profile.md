...
| --- | --- | --- |
| `complexity` | top-level list items under `## Changes`; indented sub-points belong to their item | ≤3 `low`, ≤7 `medium`, else `high` |
| `context` | distinct backticked paths under `## Context` (a token with a `/`, or a file extension) | ≤3 `small`, ≤8 `medium`, else `large` |
...
A policy is one module in `engine/policies/` exporting
`select(estimate, { session })`, where `session` is `implementation` or
`review`. The registry finds policies by file name. No scheduler, claim,
