...
2. Run `orchestrate status` and find the key's row.
   ...
   - `human-gate`: the gate must be resolved first. If the human has answered
     in this conversation, run `orchestrate gate <key> resolve --answer "<answer>"`
     and continue. Otherwise, restate the question and stop.
   - `stale` with no worktree (branch only, or a claim ref only): nothing is
     here to resume. Report it, and leave releasing it to the human through
